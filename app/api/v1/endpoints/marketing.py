...
async def _execute_send(
    ...
    *,
    ...
    # Track which users AND push tokens we've sent to in last 24 hours (prevent duplicates)
    # 24-hour window ensures only 1 marketing notification per user per day max
    ...
    for user in recipients:
        if not user.push_token or str(user.id) in recently_sent_users:
            _log.debug("Skipping user %s - already sent in last 24h", user.id)
            continue
        # CRITICAL: Skip if this push token received ANY notification in last 24 hours
        if user.push_token in recently_sent_tokens:
            _log.debug("Skipping user %s - push token already used in last 24h", user.id)
            continue
